...
void doPC1(byte data[])
{
	...
	//
	for(j=0; j<7; j++) {
		buf[j] |= ((data[PC1HI[j][0]]>>PC1LO[j][0]) & 1) << (7-0);
		buf[j] |= ((data[PC1HI[j][1]]>>PC1LO[j][1]) & 1) << (7-1);
		buf[j] |= ((data[PC1HI[j][2]]>>PC1LO[j][2]) & 1) << (7-2);
		buf[j] |= ((data[PC1HI[j][3]]>>PC1LO[j][3]) & 1) << (7-3);
		buf[j] |= ((data[PC1HI[j][4]]>>PC1LO[j][4]) & 1) << (7-4);
		buf[j] |= ((data[PC1HI[j][5]]>>PC1LO[j][5]) & 1) << (7-5);
		buf[j] |= ((data[PC1HI[j][6]]>>PC1LO[j][6]) & 1) << (7-6);
		buf[j] |= ((data[PC1HI[j][7]]>>PC1LO[j][7]) & 1) << (7-7);
	}
	memcpy(data, buf, 8);
}
